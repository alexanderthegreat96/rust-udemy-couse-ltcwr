...
fn score_points(points: &mut i32, value: i32) -> () {
    *points += value;
}

fn main() {
    let season: &str = "Winter";
    let mut points_scored: i32 = 0;

    score_points(&mut points_scored, 35);

    let event_time: &str = "06:00";
    let event_time: i32 = 6;

    println!(
        "The season is {} and we managed to score {} points. We have {} touchdowns after {} hours. I cannot believe we did this during {0}.",
        season, points_scored, TOUCHDOWN_POINTS, event_time
    );

    let favorite_beverage: &str = "cola";
}
